...
def test_current_connect_classification_is_reported(baseline_mission, record_property):
    ...
    for key in ("topology", "free_flight_between_components", "free_flight_endpoint_access", "point_access", "unexpected_fallback", "unknown"):
...
def _image_context(mission):
    ...
    for item in effective_image_points(mission):
        point = copy.deepcopy(item)
        detection = point.get("detection_result") or {}
        raw = detection.get("raw_coord") or point.get("raw_coord") or point.get("pixel_position")
        snapped = detection.get("snapped_coord") or point.get("snapped_coord") or raw
        point["raw_coord"] = list(raw[:2])
        point["snapped_coord"] = list(snapped[:2])
...
def test_start_end_replan_current_behavior(baseline_mission, pure_replan):
    ...
    assert {
        ...
        for identity in map(point_identity, mission["inspection_points"])
    } == {
        identity[0]: identity[1:]
        for identity in map(point_identity, context["inspection_points"])
    }
